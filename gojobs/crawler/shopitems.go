...
func GetShopItems(shoplink string) ([]string, error) {
    ...
    if err != nil {
        log.ErrorfType(HTTP_ERR, "%s is %s", shoplink, err)
        return nil, err
    }
    ...
    if resp.StatusCode == 200 {
        ...
        if alllink.Length() < 2 {
            ...
            log.ErrorfType(SHOP_ERR, "%s is %s", shoplink, err)
            //出现这种情况一般是店铺不存在了
            return nil, err
        }
...
func getitems(alllink string) (items []string, err error) {
    ...
    resp, err := client.Do(req)
    if err != nil {
        log.ErrorfType(HTTP_ERR, "%s is %s", alllink, err)
        return nil, err
    }
    doc, err := goquery.NewDocumentFromResponse(resp)
    if err != nil {
    ...
    tds.Each(func(i int, sq *goquery.Selection) {
        ...
        if !exist {
            log.ErrorfType(SHOP_ERR, "%s is %s", alllink, err)
            return
        }
